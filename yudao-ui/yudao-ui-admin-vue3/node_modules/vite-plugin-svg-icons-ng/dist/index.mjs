...
async function process(e, cache, dir, options) {
  ...
  try {
    const relativePath = normalizePath(path2).replace(normalizePath(dir + "/"), "") || "";
...
function getWeakETag(str) {
  return str.length === 0 ? '"W/0-2jmj7l5rSw0yVb/vlWAYkK/YBwk"' : `W/${Buffer.byteLength(str, "utf8")}-${createHash("sha1").update(str, "utf8").digest("base64").substring(0, 27)}`;
}
